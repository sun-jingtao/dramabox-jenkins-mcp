...
| `get_status` | 当前配置、最近构建尝试、最近严格成功部署、触发来源、主干合并状态及 Job 描述中的部署页线索 | 否 |
...
Job 与仓库的对应关系取自 Jenkins Remote API 导出的 SCM 配置，归一化 IP/域名、`.git` 后缀、斜杠和大小写后，与 GitLab 项目路径全等比较，不按 Job 名模糊猜测。环境过滤支持 `hot`、`qat`、`qat2`，按 Job 名分隔词精确匹配，QAT 不会混入 QAT2。
